...
- Don't keep fallback shapes inside kernels,  @__init.py__ or @layers.py. The model architecture will not change and neither will the shapes of its layers. The only shape that will be variable will be the prefill shape (seq_len), since that will depend on the user prompt.
