...
def test_greedy_multiturn_generation_matches_llama_cpp():
    ...
    first_messages = [{"role": "user", "content": "Hello, my name is"}]
    first = engine.tokenizer.decode(list(session.generate(first_messages, max_new_tokens=64, thinking=False,
                                                          temperature=0.0)), skip_special_tokens=False)
    assert first.strip() == FIRST_TURN_RESPONSE, repr(first)
    messages = first_messages + [{"role": "assistant", "content": first},
                                 {"role": "user", "content": "Bautista"}]
    ours = engine.tokenizer.decode(list(session.generate(messages, max_new_tokens=64, thinking=False,
                                                          temperature=0.0)), skip_special_tokens=False)
