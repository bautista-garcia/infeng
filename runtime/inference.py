...
class InferenceEngine:
    def __init__(self, config: str | Path, weights: str | Path, tokenizer: str, device: str | torch.device = "auto", dtype: str | torch.dtype = "auto"):
    ...
    def _sample(self, logits: torch.Tensor, temperature: float = 0.0, top_p: float = 1.0, top_k: int | None = None) -> torch.Tensor:
        if temperature <= 0:
    ...
    @torch.no_grad()
    def generate(self, prompt: str, max_new_tokens: int | None = None, thinking: bool = False, stop_token_ids: list[int] | None = None, temperature: float = 0.0, top_p: float = 1.0, top_k: int | None = None):
        text = self.tokenizer.apply_chat_template([{"role": "user", "content": prompt}], tokenize=False, add_generation_prompt=True, enable_thinking=thinking) if hasattr(self.tokenizer, "apply_chat_template") else prompt
        tokens = self.tokenizer(text, return_tensors="pt").input_ids.to(self.device)
        stop_token_ids = [t for t in (self.tokenizer.eos_token_id, self.tokenizer.convert_tokens_to_ids("<|im_end|>")) if t is not None] if stop_token_ids is None else stop_token_ids
        max_new_tokens = self.model.config.get("max_position_embeddings", 4096) - tokens.shape[1] if max_new_tokens is None else max_new_tokens
        cache = None
